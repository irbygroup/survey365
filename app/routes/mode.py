...
async def start_known_base(req: KnownBaseRequest):
    ...
    async with _mode_lock:
        ...
        await start_base(gnss_manager, site["lat"], site["lon"], height, mode="known_base")
...
async def _run_relative_base(duration: int):
    ...
    async with _mode_lock:
        ...
        # Configure GNSS receiver and start RTCM outputs
        await start_base(gnss_manager, avg_lat, avg_lon, avg_height, mode="relative_base")
...
async def _run_cors_establish(
    ...
    async with _mode_lock:
        ...
        await start_base(gnss_manager, avg_lat, avg_lon, avg_height, mode="known_base")
